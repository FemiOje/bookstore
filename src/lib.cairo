...
mod Bookstore {
    ...
    impl BookstoreImpl of IBookstore<ContractState> {
        fn add_book(
            ...
            self.all_books.entry(book_id).write(new_book);
        }

        fn remove_book(ref self: TContractState, book_id: u256) {
            self.all_books.entry(book_id).write(0); // might throw error if Book default value is not 0
        ...
        fn get_book_by_id(self: @TContractState, book_id: u256) -> Book {
            let result = self.all_books.entry(book_id).read();
            result
        }

        fn buy_book(ref self: TContractState, book_id: u256) {
            let mut caller = get_caller_address();
            let old_book = self.all_books.entry(book_id).read();
